...
def eval(model, atom_encoder, device, loader, evaluator):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if batch.x.shape[0] == 1:
            ...
            node_dimension_attack_ratio = 0.05
            edge_mask_array = np.load("/usr/scratch/hchen799/gnn_explainer/GNN_Acc-main/GCN/Golden_GCN/edge_mask/edge_mask_graph_%d.npy" % step)
            node_mask_array = np.load("/usr/scratch/hchen799/gnn_explainer/GNN_Acc-main/GCN/Golden_GCN/node_mask/node_mask_graph_%d.npy" % step)
            
            hidden_dimension = node_mask_array.shape[1]
            attacked_node_feature_num = int(node_dimension_attack_ratio * hidden_dimension)
