...
def eval_with_explainer(model, atom_encoder, device, loader, evaluator):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if batch.x.shape[0] == 1:
            ...
            np.save("./edge_mask_molhiv/edge_mask_graph_%d.npy" % step, edge_mask_array)
            
            np.save("./node_mask_molhiv/node_mask_graph_%d.npy" % step, node_mask_array)
...
def main():
    ...
    model.load_state_dict(torch.load('./GCN_ogbgmol_weights_new/gcn_ep199_dim100.pt'))
    atom_encoder.load_state_dict(torch.load('./GCN_ogbgmol_weights_new/gcn_ep199_dim100_atom_encoder.pt'))
