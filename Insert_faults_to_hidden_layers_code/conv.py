...
class GNN_node(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batched_data, insert_layer_index, real_index_position_list):
        x_new, edge_index_new, edge_attr, batch = batched_data.x, batched_data.edge_index, batched_data.edge_attr, batched_data.batch

        ### computing input node embedding

        emb_dim = 300
        #self.atom_encoder = AtomEncoder2(emb_dim)
        #h_list = [self.atom_encoder(x_new)]
        h_list = [x]
        #print("the shape of embedding after encoding is:", h_list[0].shape)
        ...
        for layer in range(self.num_layer):
            ...
            if (layer == insert_layer_index):
                node_embedding = h_list[layer]
                for i in range (len(real_index_position_list)):
                    node_embedding[real_index_position_list[i]] = 0.0
                h = self.convs[layer](node_embedding, edge_index, edge_attr)
            else:    
