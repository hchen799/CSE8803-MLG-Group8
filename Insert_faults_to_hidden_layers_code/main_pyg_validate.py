...
def bit_flipping_float(input_value, flip_bit_num):
    value_binary = float_to_binary32(input_value)
    random_index = np.random.randint(0, 32, flip_bit_num)
    flipped_binary = value_binary
    for i in range (len(random_index)):
        flipped_binary = flip_bit(flipped_binary, random_index[i])
    flipped_value = binary32_to_float(flipped_binary)
...
def eval(model, atom_encoder, device, loader, evaluator):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if batch.x.shape[0] == 1:
            ...
            node_dimension_attack_ratio = 0.75
            edge_mask_array = np.load("/usr/scratch/hchen799/gnn_explainer/GNN_Acc-main/GCN/Golden_GCN/edge_mask_molhiv/edge_mask_graph_%d.npy" % step)
            node_mask_array = np.load("/usr/scratch/hchen799/gnn_explainer/GNN_Acc-main/GCN/Golden_GCN/node_mask_molhiv/node_mask_graph_%d.npy" % step)
            
            hidden_dimension = node_mask_array.shape[1]
            node_num = node_mask_array.shape[0]
            attacked_node_feature_num = int(node_dimension_attack_ratio * hidden_dimension * node_num)
            zero_indices = np.argwhere(node_mask_array == 0)

            # Iterate through the zero indices and replace them with random numbers
            for index in zero_indices:
                random_number = random.randint(1, 10)/100.0  # Generate a random number between 1 and 10 (you can adjust the range)
                #print(random_number)
                node_mask_array[index[0]][index[1]] = random_number
            #indices = np.argsort(node_mask_array, axis=None)[::-1]
            indices = np.argsort(node_mask_array, axis=None)
            
            node_mask_shape = node_mask_array.shape
            real_index_position_list = []
            for i in range (attacked_node_feature_num):
                real_index_position = get_actual_index(indices[i], node_mask_shape)
                real_index_position_list.append(real_index_position)
                
            # real_index_position = get_actual_index(indices[0], node_mask_shape)
            # node_mask_max = node_mask_array[real_index_position]
            # node_mask_max_2 = np.max(node_mask_array)
            # print("the value of the node mask max is:", node_mask_max)
            # print("the value of the node mask max 2 is:", node_mask_max_2)
            
            
            # Now let's insert the attack
            # for i in range (len(real_index_position_list)):
            #     node_features[real_index_position_list[i]] = 0.0
            # # If it is bit flipping attack
            # for i in range (len(real_index_position_list)):
            #     node_features[real_index_position_list[i]] = bit_flipping_float(node_features[real_index_position_list[i]], 1)
            
            
            insert_layer_index = 2
            pred = model(node_features, edge_index, batch, insert_layer_index, real_index_position_list)
...
def main():
    ...
    model.load_state_dict(torch.load('/export/hdd/scratch/hchen799/gnn_explainer/GNN_Acc-main/GCN/Golden_GCN/GCN_ogbgmol_weights_new/gcn_ep199_dim100.pt'))
    atom_encoder.load_state_dict(torch.load('/export/hdd/scratch/hchen799/gnn_explainer/GNN_Acc-main/GCN/Golden_GCN/GCN_ogbgmol_weights_new/gcn_ep199_dim100_atom_encoder.pt'))
